...
def main():
    ...
    for epoch in range(total_epochs):
        ...
        if final_epoch_reached + 1 > warmup_epochs:
            early_stopping(epoch_val_loss[0], custom_fasterrcnn.model, epoch, optimiser, save_dir)
    ...
    custom_fasterrcnn.model.load_state_dict(
        torch.load(join(save_dir, 'model_best.pth'), weights_only=True)['model_state_dict'])
    custom_fasterrcnn.model.eval()
    ...
    with torch.no_grad():
        ...
        for images, targets in val_loader:
            ...
            fasterrcnn_utils.plot_validation_results(detections, images, 1, counter, save_dir)
