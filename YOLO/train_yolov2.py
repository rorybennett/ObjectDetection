...
def main():
    ...
    ############################################################################################################################################################
    yolo_model.load_state_dict(torch.load(join(save_dir, 'model_latest.pth'), weights_only=True)['model_state_dict'])
    yolo_model.eval()
    ...
    with torch.no_grad():
        ...
        for index, (images, _) in enumerate(val_loader):
            ...
            yolo_utils.plot_yolov2_validation_results(detections, images, 13, train_anchors, counter, train_mean, train_std, save_dir)
