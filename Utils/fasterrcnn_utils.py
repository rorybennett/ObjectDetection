...
def plot_validation_results(validation_detections, validation_images, detection_count, counter,
                            save_dir):
    """
        ...
        for label, label_results in highest_scoring_boxes.items():
            ...
            for j, s in enumerate(scores):
                ...
                patch = patches.Rectangle((box[0], box[1]), box[2] - box[0], box[3] - box[1], linewidth=1,
                                          edgecolor=box_colours[label - 1], facecolor='none')
                ax.add_patch(patch)
                ax.text(box[0], box[1], f'{label}: {s:0.1f}', ha='left', color=box_colours[label - 1],
                        weight='bold', va='bottom')

        plt.savefig(join(save_dir, f'val_result_{batch_number}.png'))
        plt.close()
        batch_number += 1
