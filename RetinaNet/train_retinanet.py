...
from Datasets.retinanet_dataset import ProstateBladderDataset as PBD
from EarlyStopping.EarlyStopping import EarlyStopping
from RetinaNet.CustomRetinaNet import CustomRetinaNet
from Transformers import Transformers
from Utils import general_utils, retinanet_utils
from Utils.arg_parsers import FasterRCNNArgParser
...
########################################################################################################################
arg_parser = FasterRCNNArgParser()
...
save_dir = args.save_dir  # Path to saving directory, where models, loss plots, and validation results are stored.
...
########################################################################################################################
# Set up model, optimiser, and learning rate scheduler.
########################################################################################################################
train_mean = [train_mean] * 3
train_std = [train_std] * 3
print(f'Loading RetinaNet model {backbone_type}...', end=' ')
custom_retinanet = CustomRetinaNet(num_classes=num_classes, backbone_type=backbones[backbone_type], min_size=image_size,
                                   max_size=image_size, image_mean=train_mean, image_std=train_std)
...
def main():
    ...
    for epoch in range(total_epochs):
        ...
        if final_epoch_reached + 1 > warmup_epochs:
            early_stopping(epoch_val_loss[0], custom_retinanet.model, epoch, optimiser, save_dir)

        retinanet_utils.plot_losses(early_stopping.best_epoch + 1, training_losses, val_losses, training_learning_rates,
                                    save_dir)
    ...
    ####################################################################################################################
    custom_retinanet.model.load_state_dict(torch.load(join(save_dir, 'model_best.pth'),
    ...
    with torch.no_grad():
        ...
        for images, targets in val_loader:
            ...
            retinanet_utils.plot_validation_results(detections, images, 1, counter, save_dir)
